...
def model2trend(NE_model, mlp_model): #include mutation
    ...
    result = np.concatenate((y_pred_NE, y_pred, y_all_data))
    result = result.reshape(-1, (len(y_pred_minmax)))
    result = result.transpose()
    result = pd.DataFrame(result)
    result = pd.concat([raw_x_all_data, result], axis=1)
    result_header = ["dopant", "Cdose1(cm^-2)","Cdose2(cm^-2)","Edose(cm^-2)", "DoseEnergy1(KeV)", "DoseEnergy2(KeV)", "prediction_IC_NE", "prediction_IC_MLP", "IC_0.5V(Ib_2e-6) "]
...
def run_trend_gen(data_num):
    ...
    for i in range(num_line):
        ...
        for j in range(0, dataset, sample_rate):
            pred = process_csv.iloc[j:j+sample_rate]['prediction_IC_MLP']
            true = process_csv.iloc[j:j+sample_rate]['IC_0.5V(Ib_2e-6) ']
