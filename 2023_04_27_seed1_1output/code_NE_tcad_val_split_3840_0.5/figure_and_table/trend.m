...
for i = 1:figure_num
    ...
    hold on
    set(gca,'FontSize', 20, 'LineWidth', 4.0)
    xlim([4.5E15, 11E15]),ylim([(15 / 16 * y_min) (y_max + y_min / 8)])%,xticks(0:0.3:0.9), yticks(0:5e-4:1e-3);
    ...
    hold on
    set(gca,'FontSize', 20, 'LineWidth', 4.0,'YScale','log');
    xlim([4.5E15, 11E15]),ylim([(15 / 16 * y_min) (y_max + y_min / 8)])%ylim([(y_min/2) (y_max + y_min)])%,xticks(0:0.3:0.9), yticks(0:5e-4:1e-3);
    xlabel('\bfCdose 2(cm^-^2)', 'FontSize',20), ylabel('\bfI_c (A) ', 'FontSize',20)
    legend([pred_mlp, pred_NE, simulate], {'\bfMLP', '\bfNE', '\bfTrue'},'FontSize',14, 'Location','northwest','Box','off');
end
